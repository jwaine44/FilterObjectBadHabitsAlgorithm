    ...
    let expected = [];
        for(let i = 0; i < persons.length; i++){
              if(persons[i].habits.includes(badHabit))
                  expected.push(persons[i].firstName + " " + persons[i].lastName)
        }
        return expected;                         
    }
...
function santasNaughtyListFunctional(persons, badHabit) {
    const naughtyPersonList = persons.filter((naughtyPerson) => naughtyPerson.habits.includes(badHabit))
    const naughtyPersonName = naughtyPersonList.map((naughtyPerson) => naughtyPerson.firstName + " " + naughtyPerson.lastName)
    return naughtyPersonName 
}
